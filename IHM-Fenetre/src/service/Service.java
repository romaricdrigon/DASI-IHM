...
public class Service {
    ...
    //Artiste déja mis à jour par ailleurs ou pas? non, aucune maj à faire
...
//         artiste.ajouterOeuvre(oeuvre);
        try {
            ...
            artisteDao.update(artiste); //Pas utile car aucun lien de artiste vers oeuvre...
            tx.commit();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    /*
     * !! Attention à la recherche, idArtiste =-1 signifie qu'on ne recherche que sur Prix et Date !!
     *
     * */
    public List<Oeuvre> rechercherOeuvreParPrixArtisteDate(int idArtiste,
        ...
        List<Oeuvre> listeOeuvres;
        JpaUtil.openEntityManager();
        //si artiste vaut -1, cela signifie que l'on ne veut pas faire de requete au niveau de l'artiste
        if (idArtiste == -1) {
    ...
    /* On vide la galerie avant qu'elle ne soit persistée
     * Pas utilisée
     * */
